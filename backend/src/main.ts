...
async function bootstrap() {
  ...
  const options = new DocumentBuilder()
    ...
    .addBearerAuth()// x (Comment x) // here is an intentional compile error. Remove the "x" and the backend should compile.
    .build();
  ...
  app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(document, options2));
